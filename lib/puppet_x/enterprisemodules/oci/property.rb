      ...
      class Property < Puppet::Property
        ...
        def fill_in_reference_in_enumerable(data)
          ...
          reference_attributes.each do |property|
            ...
            #
            # We use the ttpe if it is specfied, if not specified, we infer a type based on
        ...
        def reference_attributes
          self.class.data_type.to_alias_expanded_s.scan(/'(\w*?_ids?)'/).flatten
        end

        def reference_to_id(reference, type)
          #
          # If a name is specfied translate it to an id before the create or update
          # is called.
          #
