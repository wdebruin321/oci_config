...
describe 'oci_config', :type => :class do
  context "No gem installed" do
    ...
    it { is_expected.to contain_file('/tmp/oci-2.13.0.gem')
      .with('path'   => '/tmp/oci-2.13.0.gem')
      .with('ensure' => 'present')
      .with('source' => 'puppet:///modules/oci_config/oci-2.13.0.gem')
  ...
  context "requested version of gem already installed" do
    let(:facts) {{
      :oci_installed_gem => '2.13.0',
    }}

    it { is_expected.not_to contain_file('/tmp/oci-2.13.0.gem')}
